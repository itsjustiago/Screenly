...
enum Updater {
    static let repo = "itsjustiago/Screenly"

    static var currentVersion: String {
        if let debug = ProcessInfo.processInfo.environment["SCREENLY_DEBUG_VERSION"] { return debug }
            ...
            if let data,
               ...
               isNewer(tag, than: currentVersion) {
                ...
                if let assets = json["assets"] as? [[String: Any]] {
                    for a in assets where (a["name"] as? String) == "Screenly.zip" {
